...
<AccordionGroup>
  ...
  <Accordion title="Stats">
    Use stats when you want a compact statistical result or metric-style card.

    Good uses include quick public metric checks, small KPI summaries, and stat-card style answers when the needed values are clear.

    Try:

    ```text
    Show a compact stats card for these quarterly numbers: Q1 120, Q2 175, Q3 160, Q4 210.
    ```

    Stats is not a full spreadsheet engine. For row-by-row analysis, attach a file or use a connected spreadsheet app when available.
  </Accordion>
